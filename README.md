...
项目现在同时提供命令行界面和 Web 前端。Web 前端运行在本机回环地址，适合课堂演示、截图和人工复核。
...
- 提供 Flask Web 前端，可选择样例或上传本地日志、运行分析、查看告警详情并导出报告。
...
启动后在浏览器打开 `http://127.0.0.1:8765`。Web 前端支持：

- 选择内置样例日志或上传一个或多个本地日志文件
- 点击“开始分析”运行现有检测流程
