...
**Goal:** Add paginated finding review, Chinese/English UI switching, explicit time distribution, detailed attacker IP statistics, and local keyword/facet filtering to the Logcheck web workbench.

**Architecture:** Keep analytics derivation in `logcheck/web_static/app.js` unless existing serialized analysis data is insufficient. Add small local helpers for translation, filtering, pagination, time buckets, and source-address aggregation, then wire them into existing render functions. Keep backend behavior unchanged unless tests prove serialization needs a narrow extension.
...
- Modify `logcheck/web_static/index.html`: add language control, filter controls, pagination container, and attacker IP statistics region.
...
- Modify `logcheck/web_static/styles.css`: style language control, filters, pagination, and attacker IP table without nested cards or horizontal overflow.
...
## Task 1: Add Regression Tests For Static UI Contract
...
## Task 2: Add Markup And Styles For Controls
...
- [ ] **Step 1: Add language control in the topbar**
...
- [ ] **Step 2: Rename the time chart title**
...
- [ ] **Step 3: Add attacker IP statistics panel**

Insert this article inside `#chart-grid`, after the severity chart article:
...
- [ ] **Step 4: Add filter controls and pagination container**

In the finding queue panel, replace the current panel body:
...
Append these rules before the media queries in `logcheck/web_static/styles.css`:
...
```bash
pytest tests/test_webapp.py::test_dashboard_renders_language_filter_pagination_and_attacker_ip_regions tests/test_webapp.py::test_dashboard_time_chart_title_is_explicit tests/test_webapp.py::test_dashboard_styles_include_filters_pagination_and_attacker_ip_rules -q
```

Expected: these tests pass. Helper tests can still fail until Task 3 and Task 4.

- [ ] **Step 7: Commit markup and styles**

```bash
git add logcheck/web_static/index.html logcheck/web_static/styles.css
git commit -m "feat: add workbench analytics controls"
```
