...
- [ ] 4.2 Add normalized evidence text helper logic only if needed for useful keyword matching, preserving raw evidence display.
...
- [ ] 5.1 Add finding queue pagination controls and integrate them with selection and filters.
- [ ] 5.2 Add a compact local i18n dictionary and visible English/Chinese language control.
- [ ] 5.3 Ensure core UI labels, status text, filter labels, chart labels, empty states, and export/status messages use the language dictionary.
...
- [ ] 5.6 Add keyword and facet filter controls that filter first and paginate second.
